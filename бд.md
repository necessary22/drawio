...
Описание: Этот триггер обновляет общую стоимость заказа (total_cost) в таблице orders, когда происходят изменения в таблице orders_details (например, при добавлении, обновлении или удалении деталей заказа).

2. Триггер для проверки количества товара в корзине перед оформлением заказа:
sql
Copy code
...
Описание: Этот триггер проверяет, есть ли товары в корзине (cart), прежде чем будет создан новый заказ (orders). Если корзина пуста при попытке создания заказа, то будет сгенерировано исключение.

3. Триггер для обновления статуса заказа в зависимости от его общей стоимости:
sql
Copy code
CREATE OR REPLACE FUNCTION update_order_status()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.total_cost > 200 THEN
        NEW.status_id := 3; -- Изменяем статус на "Отправлен"
    ELSIF NEW.total_cost > 100 THEN
        NEW.status_id := 2; -- Изменяем статус на "В обработке"
    ELSE
        NEW.status_id := 1; -- Изменяем статус на "В ожидании"
    END IF;
...
Описание: Этот триггер автоматически обновляет статус заказа (status_id) в зависимости от его общей стоимости при создании нового заказа (orders).

4. Триггер для внесения изменений в заказы после обновления информации о продуктах:
sql
Copy code
CREATE OR REPLACE FUNCTION update_orders_after_product_update()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE orders_details
    SET amount = amount * NEW.price_difference
    FROM product
...
CREATE TRIGGER update_orders_after_product_update_trigger
AFTER UPDATE ON product
FOR EACH ROW
EXECUTE FUNCTION update_orders_after_product_update();
Описание: Этот триггер обновляет количество товара в заказах (orders_details.amount) после обновления цены продукта (product.price) в случае, если цена продукта изменилась.
